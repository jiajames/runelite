...
public class OneClickPlugin extends Plugin
{
	...
	private DialogQuery dialogQuery = new DialogQuery();
	...
	private void onMenuEntryAdded(MenuEntryAdded event)
	{
		...
		switch (type)
		{
			...
			case DARK_ESSENCE:
				...
				break;
			case BUILD_LARDER:
				if (event.getOption().equals("Build") && event.getTarget().toLowerCase().contains("larder"))
				{
					event.setTarget(event.getTarget() + " <col=ff9040>(easy!)<col=ffffff>");
					event.setModified();
				}
				else if (event.getOption().equals("Remove") && event.getTarget().toLowerCase().contains("larder"))
				{
					event.setTarget(event.getTarget() + " <col=ff9040>(easy!)<col=ffffff>");
					event.setModified();
				}
	...
	private void onMenuOptionClicked(MenuOptionClicked event)
	{
		...
		switch (type)
		{
			...
			case DARK_ESSENCE:
				...
				break;
			case BUILD_LARDER:
				if (option.equals("Build") && target.contains("<col=ff9040>(easy!)<col=ffffff>"))
				{
					buildLarder = true;
				}
				else if (option.equals("Remove") && target.contains("<col=ff9040>(easy!)<col=ffffff>"))
				{
					removeLarder = true;
				}
	...
	private void executeLarderAction(int id)
	{
		Random r = new Random();
		if (id == 0) // BUILD
		{
			flexo.delay(r.nextInt((1500 - 1000 + 1) + 1000));
			flexo.keyPress(KeyEvent.VK_2);
		}
		else if (id == 1) // REMOVE
		{
		...
		{
			System.out.println("not recognized larder action!");
		}
	}
